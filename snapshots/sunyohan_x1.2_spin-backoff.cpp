...
// -------------------- 주행 세팅 --------------------
const int BASE = 104;           // 기준선 87 x1.2
...
const int CURVE_IN  = -102;  // 안쪽 바퀴 (기준선 -85 x1.2)
const int CURVE_OUT = 132;   // 바깥 바퀴 (기준선 110 x1.2)
// 전 구간 주행에 성공한 기준선(sunyohan copy.cpp)에서 PWM만 1.2배.
...
// 정지 -> 출발 순간 부스트
const int START_BOOST_PWM = 216;   // 기준선 180 x1.2
const unsigned long START_BOOST_MS = 120;

// 직각 회전 기본 PWM
const int PIVOT = 162;             // 기준선 135 x1.2

// 직각 회전이 처음 시작될 때만 토크 부스트
const int PIVOT_BOOST = 210;       // 기준선 175 x1.2
const unsigned long PIVOT_BOOST_MS = 100;

// 255를 넘기면 motor()가 조용히 잘라낸다. 값을 일괄 배율할 때 여기서 걸린다.
static_assert(BASE <= 255 && -CURVE_IN <= 255 && PIVOT <= 255 &&
              PIVOT_BOOST <= 255 && START_BOOST_PWM <= 255,
              "PWM over 255 is silently clamped by motor()");

// 아래 시간 3개는 거리/각도를 시간으로 적어둔 값이라 속도에 반비례한다.
// 다만 PWM 1.44배에서 1/1.44로 "온전히" 나눴더니 주행이 깨졌다. 모터 속도는
// PWM에 비례하지 않고(L298N 전압강하, 마찰) 전진 구간은 가속 중이라 실제로는
// 덜 빨라진다. 그래서 PWM 1.2배에 대해 1/1.2가 아니라 그 절반만 보정한다(1/1.10).
//   온전히 나눈 값: 208 / 267 / 100    검증된 기준선: 250 / 320 / 120
//   코너를 지나쳐서 늦게 돌면 -> 더 줄인다 (208 쪽)
//   꼭짓점 전에 미리 돌면   -> 더 늘린다 (320 쪽)

// 한쪽 센서가 이 시간 이상 계속 검정을 보면 직각 코너로 판정
const unsigned long CORNER_MS = 227;   // 250 / 1.10

// 코너 판정 후 실제 회전을 시작하기 전에 앞으로 가는 시간
// 센서가 바퀴축보다 약 9 cm 앞에 있는 현재 구조 기준
const unsigned long ADVANCE_MS = 291;   // 320 / 1.10

// 회전 종료 후 반대로 살짝 되감는 시간
const unsigned long BACKOFF_MS = 200;   // 120 / 1.10
...
Drive decide(bool L, bool R, unsigned long now) {
  ...
  if (cornerDir) {
    ...
    if (now < backUntil && !detector) {
      // 되감기는 제자리 회전. 양 바퀴를 반대로 돌려 회전축을 차체 중심에 둔다.
      // (좌우 크기가 같아야 제자리가 되므로 applyStartBoost가 한쪽만 올리면 안 된다)
      return (cornerDir < 0)
               ? mk(PIVOT, -PIVOT)
               : mk(-PIVOT, PIVOT);
    }
...
//
// halted = 두 바퀴가 모두 0. 한쪽 바퀴만 0인 것은 제자리 회전의 정상 동작이지
// 정지가 아니다. 그걸 정지로 보면 회전이 끝날 때마다 그 바퀴가 다음 명령에서
// START_BOOST_PWM으로 튀어 되감기와 코너 복귀가 좌우 비대칭이 된다.
int applyStartBoost(
  int v,
  bool halted,
  int8_t &lastSign,
...
void loop() {
  ...
  if (LOG_MS && now - lastLog > LOG_MS) {
    ...
    // 직선에서 corner가 0이 아니면 코너 오검출, 0인데 못 따라가면 보정 진동이다.
    Serial.print(F("corner="));
    Serial.print(cornerDir);
    Serial.print(F("\t"));
